// 默认的回调函数，用于关闭弹出窗口
var _defaultAjaxCallbackSuccess = function(return_data){
...
var _defaultAjaxCallbackError = function(return_data) {
...
function ajaxPost(form, callbackSuccess, callbackError) {
    return $.ajax({
        ...
        success: function(return_data) {
            callbackSuccess ? callbackSuccess(return_data) : _defaultAjaxCallbackSuccess(return_data);
        },
        error: function(return_data) {
            var json = JSON.parse(return_data.responseText);
            callbackError ? callbackError(json) : _defaultAjaxCallbackError(json);
        }
    });
}
